package com.quantum.marchofents.blocks;
...
public class MOECraftingTableFangorn extends LOTRBlockCraftingTable {
	
	@SideOnly(Side.CLIENT)
	private IIcon[] tableIcons;

	public MOECraftingTableFangorn() {
		super(Material.rock, LOTRFaction.FANGORN, MOEGuiId.FANGORN.ordinal());
	...
	public boolean onBlockActivated(World world, int i, int j, int k, EntityPlayer entityplayer, int side, float f, float f1, float f2) {
		...
		if (hasRequiredAlignment) {
			if (!world.isRemote) {
				entityplayer.openGui(MarchOfEnts.instance, tableGUIID, world, i, j, k);
			}
		} else {
			for (int l = 0; l < 8; ++l) {
	...
	public static class Container extends LOTRContainerCraftingTable {
		public Container(InventoryPlayer inv, World world, int i, int j, int k) {
			super(inv, world, i, j, k, MOERecipes.fangorn, MOEItems.fangornCraftingTable);
		}
	}
	...
	public static class Gui extends LOTRGuiCraftingTable {
		public Gui(InventoryPlayer inv, World world, int i, int j, int k) {
			super(new Container(inv, world, i, j, k), "fangorn");
		}
	}
